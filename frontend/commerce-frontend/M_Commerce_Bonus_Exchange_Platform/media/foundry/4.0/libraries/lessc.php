...
class FD40_lessc {
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new FD40_lessc_parser($this, __METHOD__);
	...
	public function makeParser($name) {
		$parser = new FD40_lessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "FD40_lessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "FD40_lessc_formatter_$this->formatterName";
		}
...
class FD40_lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('FD40_lessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = FD40_lessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = FD40_lessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = FD40_lessc::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function FD40_lessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by FD40_lessc::match(), which behaves
	 ...
	 * avoid this, FD40_lessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("FD40_lessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = FD40_lessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = FD40_lessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("FD40_lessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = FD40_lessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.FD40_lessc::preg_quote($what), $m, !$until)) return false;
...
class FD40_lessc_formatter_classic {
	public $indentChar = "  ";
...
class FD40_lessc_formatter_compressed extends FD40_lessc_formatter_classic {
...
class FD40_lessc_formatter_lessjs extends FD40_lessc_formatter_classic {
	public $disableSingle = true;
