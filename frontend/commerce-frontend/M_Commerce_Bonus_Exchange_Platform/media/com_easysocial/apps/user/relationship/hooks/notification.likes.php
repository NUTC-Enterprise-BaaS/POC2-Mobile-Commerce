...
class SocialUserAppRelationshipHookNotificationLikes
{
    public function execute($item)
    {
        ...
        if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
            $item->title = JText::sprintf('APP_USER_RELATIONSHIP_USER_LIKES_YOUR_RELATIONSHIP_STATUS' . $plurality, $names);

            return $item;
        }

        if ($stream->actor_id == $item->actor_id && count($users) == 1) {
            $item->title = JText::sprintf('APP_USER_RELATIONSHIP_OWNER_LIKES_RELATIONSHIP_STATUS' . FD::user($stream->actor_id)->getGenderLang(), $names);

            return $item;
        }

        $item->title = JText::sprintf('APP_USER_RELATIONSHIP_USER_LIKES_USER_RELATIONSHIP_STATUS' . $plurality, $names, FD::user($stream->actor_id)->getName());
