...
<li class="reply-item<?php echo $answer && $answer->id == $reply->id ? ' is-answer-item' : ''; ?>" data-reply-item data-id="<?php echo $reply->id; ?>">
    ...
    <?php if ($event->isAdmin() || $this->my->isSiteAdmin() || $reply->created_by == $this->my->id || $question->created_by == $this->my->id) { ?>
    <div class="pull-right btn-group">
        ...
        <ul class="dropdown-menu dropdown-menu-user messageDropDown">
            <?php if (($question->created_by == $this->my->id || $this->my->isSiteAdmin() || $event->isAdmin()) && (!$answer || ($answer && $reply->id != $answer->id))) { ?>
            <li>
                <a href="javascript:void(0);" data-reply-accept-answer><?php echo JText::_('APP_EVENT_DISCUSSIONS_ACCEPT_ANSWER'); ?></a>
            </li>
            <li class="divider"></li>
            <?php } ?>

            <?php if ($reply->created_by == $this->my->id || $this->my->isSiteAdmin() || $event->isAdmin()) { ?>
            <li>
                <a href="javascript:void(0);" data-reply-edit><?php echo JText::_('APP_EVENT_DISCUSSIONS_EDIT_REPLY'); ?></a>
            </li>
            <li>
                <a href="javascript:void(0);" data-reply-delete><?php echo JText::_('APP_EVENT_DISCUSSIONS_DELETE_REPLY'); ?></a>
            </li>
            <?php } ?>
        </ul>
    </div>
    ...
    <div class="media">
        ...
        <div class="media-body">
            ...
            <form data-reply-form class="reply-form reply-content-edit">
                ...
                <?php echo FD::bbcode()->editor('reply_content', $reply->content, array('files' => $files, 'uid' => $event->id, 'type' => SOCIAL_TYPE_EVENT, 'controllerName' => 'events'), array('data-reply-content' => '')); ?>
