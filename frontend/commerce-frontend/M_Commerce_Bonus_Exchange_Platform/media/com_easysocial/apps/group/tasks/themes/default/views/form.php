	...
	<div class="es-content create-task">
		<div class="es-content-wrap">
			<form action="<?php echo JRoute::_( 'index.php' );?>" method="post" class="form">
				...
				<?php if( $milestone->id ){ ?>
					<h3><?php echo JText::_( 'APP_GROUP_TASKS_EDITING_MILESTONE' ); ?></h3>
				<?php } else { ?>
					<h3><?php echo JText::_( 'APP_GROUP_TASKS_CREATE_MILESTONE' ); ?></h3>
				<?php } ?>
				...
				<div>
					<div class="form-group">
						<input type="text" name="title" value="<?php echo $this->html( 'string.escape' , $milestone->title );?>"
							placeholder="<?php echo JText::_('APP_GROUP_TASKS_MILESTONE_TITLE_PLACEHOLDER' , true );?>" class="form-control input-sm "
						/>
					</div>
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<?php echo $this->html( 'form.calendar' , 'due' , $milestone->due , 'due' , array( 'placeholder="' . JText::_( 'APP_GROUP_TASKS_DUE_DATE_FOR_MILESTONE' , true ) . '"' ) ); ?>
						</div>
					</div>

					<div class="mt-15" style="clear:both;">
						<div class="editor-wrap fd-cf">
							<?php echo FD::bbcode()->editor( 'description' , $milestone->description , array( 'uid' => $group->id , 'type' => SOCIAL_TYPE_GROUP ) ); ?>
						</div>
					</div>
				</div>
