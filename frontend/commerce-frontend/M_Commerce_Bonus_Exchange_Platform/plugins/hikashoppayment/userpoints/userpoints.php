...
class plgHikashoppaymentUserpoints extends hikashopPaymentPlugin {
	...
	function getCartUsedPoints(&$cart) {
		...
		$q = 'SELECT * FROM '.hikashop_table('cart_product').' WHERE cart_id = '.$database->Quote($cart->cart_id);
		$database->setQuery($q);
		...
		foreach ($field as $fields) {
			...
			foreach ($price as $prices) {
				// $priceClass = str_replace(',', '', $prices->price_access);
				$totalPoint = 0;
				$originPrice = $prices->price_value;
				$point[] = ($originPrice - $prices->price_value*(($prices->price_value_with_discount/10)))*$fields->cart_product_quantity;
		...
		foreach($ids as $id) {
			...
			if($check !== false && $check > 0) {
				...
				// var_dump($this->payment_params->points_mode);
				return array(
					'points' => $totalPoint,   //扣除點數
					'value' => $totalPoint,		//扣除點數
					'mode' => $this->payment_params->points_mode //用EasySocial模式
				);
	...
	function getUserPoints($cms_user_id = null, $mode = 'all') { //選完付款方式 使用者點數
		$plugin = hikashop_import('hikashop', 'userpoints');
	...
	function checkRules($order, &$userPoints) {
		...
		$database	= JFactory::getDBO();
		$q = 'SELECT * FROM '.hikashop_table('cart_product').' WHERE cart_id = '.$database->Quote($order->cart_id);
		$database->setQuery($q);
		$field = $database->loadObjectList();  //抓取cart_product資料表
		foreach ($field as $fields) {
			$arr = 'SELECT * FROM '.hikashop_table('price').' WHERE price_product_id = '.$database->Quote($fields->product_id);
			...
			foreach ($price as $prices) {
				$totalPoint = 0;
				$originPrice = $prices->price_value;
				$point[] = ($originPrice - $prices->price_value*(1-($prices->price_value_with_discount/100)))*$fields->cart_product_quantity;
				$totalPoint = array_sum($point); //總共需付點數
			}
		}
		...
		if($check === false || $check == 0 || ($userPoints == 0 && $virtualPoints == 0) ) //判斷點數是否夠買商品
			return false;
	...
	function pointsToCurrency($userPoints) { 	//買家點數
		if(empty($this->plugin_params))
			return false;
	...
	function finalPriceToPoints(&$order, &$userPoints, $additional = true) {
		...
		if(isset($order->order_subtotal) && isset($order->order_shipping_price)) {
			...
			if(!empty($order->cart->additional['userpoints']->price_value_with_tax)){
				// var_dump($order->cart->additional['userpoints']->price_value_with_tax);
	...
	function onPaymentConfiguration(&$element) {
		...
		$this->modes[] = JHTML::_('select.option', 'jbusiness', 'JBusiness_userpoints'); //新增JBusiness_userpoints付款方式
...
';
	...
	function onPaymentConfigurationSave(&$element) {
		...
		if($element->payment_params->points_mode == 'jbusiness' ) {
			$element->payment_params->points_mode = 'jbusiness';
		}
	}
