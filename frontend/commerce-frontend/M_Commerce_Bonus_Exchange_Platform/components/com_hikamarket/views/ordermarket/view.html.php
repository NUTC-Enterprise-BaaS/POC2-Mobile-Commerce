...
class ordermarketViewordermarket extends HikamarketView {
	...
	public function listing($tpl = null) {
		...
		$this->toolbar = array(
			...
			'request' => array(
				...
				'url' => hikamarket::completeLink('order&task=request'),
				'pos' => 'right',
				'display' => ($vendor->vendor_id > 1),
	...
	public function show($tpl = null, $toolbar = true) {
		...
		$address_mode = JRequest::getInt('address_mode', 0);
		...
		if($order->order_type == 'subsale') {
			...
			if($vendors_in_cart == 1 && ((int)$config->get('vendor_edit_order', 0) == 1)) {
				$editable_order = hikamarket::isEditableOrder($order_id, (int)$order->order_vendor_id);
			}
		}
		...
		$dispatcher = JDispatcher::getInstance();
		$dispatcher->trigger('onMarketOrderEditionLoading', array(&$order) );
		...
		if($toolbar) {
			...
			$this->toolbar = array(
				...
				'order-status' => array(
					...
					'url' => hikamarket::completeLink('order&task=status&cid='.(int)$order->order_id, true),
				...
				'email' => array(
					...
					'url' => hikamarket::completeLink('order&task=mail&cid='.(int)$order->order_id, true),
				...
				'invoice' => array(
					...
					'url' => hikamarket::completeLink('order&task=invoice&type=full&cid='.(int)$order->order_id, true),
				...
				'shipping-invoice' => array(
					...
					'url' => hikamarket::completeLink('order&task=invoice&type=shipping&cid='.(int)$order->order_id, true),
	...
	public function showblock($tpl = null) {
		...
		if(isset($translate_edit[$block]) && JRequest::getVar('blocksubmitted', 0) === 1)
		...
		if(isset($addresses_blocks['edit'][ $block ])) {
			...
			$this->edit_address_mode = JRequest::getCmd('address_mode', '');
		}
	...
	public function request() {
		...
		foreach($data as $d) {
			...
			if($vendor->vendor_currency_id == $d->currency)
				$total->value += hikamarket::toFloat( (int)$d->value );
			else
				$total->value += $this->currencyClass->convertUniquePrice((float)hikamarket::toFloat($d->value), (int)$d->currency, (int)$vendor->vendor_currency_id);
		}
