...
<?php } ?>
		...
		<dd class="hikamarket_order_product_price">
			...
			<div>
				<span id="hikamarket_order_<?php echo $this->order->order_id; ?>_orderproduct_<?php echo $pid; ?>_value_price"><?php echo @$orderProduct->order_product_price; ?></span>
				+
				<span id="hikamarket_order_<?php echo $this->order->order_id; ?>_orderproduct_<?php echo $pid; ?>_value_tax"><?php echo @$orderProduct->order_product_tax; ?></span>
			</div>
...
?>
		...
		<dd class="hikamarket_order_product_customfield hikamarket_order_product_customfield_<?php echo $fieldName; ?>"><span><?php
			if($editCustomFields) {
				echo $this->fieldsClass->display($oneExtraField, @$this->orderProduct->$fieldName, 'order[products]['.$pid.'][field]['.$fieldName.']',false,'',true);
			} else {
...
?>
		...
		<dd class="hikamarket_orderproduct_vendor"><?php
			echo $this->nameboxType->display(
				...
				(int) @$orderProduct->vendor_data->order_vendor_id,
				hikamarketNameboxType::NAMEBOX_SINGLE,
				'vendor',
				array(
		...
		<dd class="hikamarket_orderproduct_vendorprice">
			<input type="text" name="order[products][<?php echo $pid; ?>][vendor_price]" id="hikamarket_order_<?php echo $this->order->order_id; ?>_orderproduct_<?php echo $pid; ?>_vendorprice" value="<?php echo @$orderProduct->vendor_data->order_product_vendor_price; ?>"/>
		</dd>
<?php
	}
?>
	...
	<div style="float:right">
		<button onclick="return window.orderMgr.submitProduct(this, <?php echo $pid; ?>);" class="btn btn-success"><img src="<?php echo HIKAMARKET_IMAGES; ?>icon-16/save2.png" alt="" style="vertical-align:middle;"/> <?php echo JText::_('HIKA_OK'); ;?></button>
	</div>
	<button onclick="return window.orderMgr.refreshProduct(this, <?php echo $pid; ?>);" class="btn btn-danger"><img src="<?php echo HIKAMARKET_IMAGES; ?>icon-16/cancel.png" alt="" style="vertical-align:middle;"/> <?php echo JText::_('HIKA_CANCEL'); ;?></button>
