...
if(empty($this->ajax)) {
	if($this->editable_order && hikamarket::acl('order/edit/products')) {
		$dropData = array(
			array(
				'name' => JText::_('HIKAM_ADD_NEW_PRODUCT'),
				'link' => '#new-product',
				'click' => 'return window.orderMgr.addProduct(this);'
			)
		);
		echo $this->dropdownHelper->display(JText::_('HIKAM_EDIT'), $dropData, array('type' => '', 'right' => false, 'up' => false));
...
window.orderMgr.refreshProduct = function(el, id, edit) {
	...
	var url = '<?php echo hikamarket::completeLink('order&task=showblock&block=HIKAACTION&cid='.(int)$this->order->order_id.'&pid=HIKAPID&tmpl=ajax', false, false, true); ?>'.replace('HIKAACTION', edit ? 'edit_product' : 'product').replace("HIKAPID", id);
