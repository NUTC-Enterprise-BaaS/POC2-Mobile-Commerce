...
?><?php
$cart = $this->initCart();  //取得cart資料表   $cart->cart_id是cartid
...
?>
				<tr id="hikashop_checkout_cart_additional_<?php echo str_replace(' ','_',$k); ?>_line" >
					...
					<td id="hikashop_checkout_cart_additional_<?php echo str_replace(' ','_',$k); ?>_title" class="hikashop_cart_additional_title hikashop_cart_title"><?php
						echo JText::_($additional->name); //使用點數
					?></td>
...
<?php
				...
				// 	if(bccomp($taxes,0,5)==0 || !$this->params->get('price_with_tax')){
				// 		echo $this->currencyHelper->format(@$totalPoint,$additional->price_currency_id); //點數抵用 放入點數
				// 	}else{
				// 		echo $this->currencyHelper->format(@$totalPoint,$additional->price_currency_id);
				// 	}
				// } else {
				// 	echo $totalPoint;    //
				// }
				...
				if(!empty($additional->price_value) || empty($additional->value)) {
					...
					if ($plugin->getUserPoints($this->row->user_id)["jbusiness"] < $totalPoint) {
						echo $plugin->getUserPoints($this->row->user_id)["jbusiness"];
					} else {
						echo $totalPoint;  //使用點數
					}
				}
...
?>
				<tr>
					...
					<td class="hikashop_cart_total_value" data-title="<?php echo Jtext::_('HIKASHOP_TOTAL'); ?>">
						...
						<?php
							$use_coupon = 1 - (int)$app->getUserState(HIKASHOP_COMPONENT.'.userpoints_no_virtual_coupon', (int)(@$this->plugin_options['checkout_step'] && @$this->plugin_options['default_no_use'])); // 取得現在選擇是否點數扣款
							$totalPrice = $this->total->prices[0]->price_value_with_tax - $totalPoint; //如果選是 扣掉點數後要付的價錢
							$plugin = hikashop_import('hikashop', 'userpoints');

							// if ($plugin->getUserPoints($this->row->user_id)["esp"] < $totalPoint) {
							if ($plugin->getUserPoints($this->row->user_id)["esp"] == 0){
								  $use_coupon = 0;
								}
							// }
							// if ($plugin->getUserPoints($this->row->user_id)["jbusiness"]) { //是否為jb付款方式
							  if ($plugin->getUserPoints($this->row->user_id)["jbusiness"] == 0) {
								  $use_coupon = 0;
								}
							// }
							if ($use_coupon == 1) {
								if ($plugin->getUserPoints($this->row->user_id)["jbusiness"] < $totalPoint) {
									echo '<p>'.'('.$plugin->getUserPoints($this->row->user_id)["jbusiness"].'點)'.$this->currencyHelper->format( $this->total->prices[0]->price_value_with_tax-$plugin->getUserPoints($this->row->user_id)["jbusiness"], $this->total->prices[0]->price_currency_id ) . '</p>'; //顯示扣掉剩餘的點數
									echo 'PV值：' . ($this->total->prices[0]->price_value_with_tax - $plugin->getUserPoints($this->row->user_id)["jbusiness"]) * 0.1;
								} else {
									echo '<p>'.'('.$totalPoint.'點)'.$this->currencyHelper->format($totalPrice, $this->total->prices[0]->price_currency_id) . '</p>'; //打完折後總和總合 和點數
									if ($userGroup->group_id == '17' || $userGroup->group_id == '18' || $userGroup->group_id == '19' || $userGroup->group_id == '20') {
										echo 'PV值：' . $totalPrice * 0.1;
									}
								}
							} else {
								echo '<p>'.$this->currencyHelper->format($this->total->prices[0]->price_value_with_tax, $this->total->prices[0]->price_currency_id) . '</p>'; //沒有使用點數折價的總和
							}
						?>
						</span>
					</td>
