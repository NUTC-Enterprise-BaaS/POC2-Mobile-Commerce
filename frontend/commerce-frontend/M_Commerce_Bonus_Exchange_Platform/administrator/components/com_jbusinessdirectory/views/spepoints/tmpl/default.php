...
<form action="<?php echo JRoute::_('index.php?option=com_jbusinessdirectory&view=spepoints');?>" method="post" name="adminForm" id="adminForm">
    ...
    <table class="table table-striped adminlist" id="itemList">
        <thead>
            <tr>
                ...
                <th nowrap="nowrap" width='23%' ><?php echo JHtml::_('grid.sort', '特約店家名稱', 'p.name', $listDirn, $listOrder); ?></th>
                ...
                <th nowrap="nowrap" width='10%' ><?php echo JHtml::_('grid.sort', '發出點數', '', $listDirn, $listOrder); ?></th>
        ...
        <?php
               ...
               $query = $db->getQuery(true);
                ...
                foreach ($pointHistorys as $key => $pointHistory) {
                    if (!isset($speSendPoint[$pointHistory->user_id])) {
                        $speSendPoint[$pointHistory->user_id] = 0;
                    }
                    $speSendPoint[$pointHistory->user_id] = $speSendPoint[$pointHistory->user_id] + abs($pointHistory->points);
                }
                //取出特約店家會員
                $db = JFactory::getDbo();
                $query = $db->getQuery(true);
                ...
                ->where($db->quoteName('b.shop_class') . ' = ' . 1);
                $db->setQuery($query);
                $userSpes = $db->loadObjectList();
                foreach ($userSpes as $key => $userSpe) {
                    $speStoreEmail[$userSpe->id] = $userSpe->email;
                    $speStoreName[$userSpe->id] = $userSpe->name;
                }
                //取出特約店家會員---推薦人
                $db = JFactory::getDbo();
                $query = $db->getQuery(true);
                $query
                ->select(array('a.name', 'a.id', 'b.phone'))
                ->from($db->quoteName('#__jbusinessdirectory_companies', 'a'))
                ->join('INNER', $db->quoteName('#__jbusinessdirectory_users', 'b') . ' ON (' . $db->quoteName('a.group') . ' = ' . $db->quoteName('b.id') . ')')
                ->where($db->quoteName('a.shop_class') . ' = ' . 1);
                $db->setQuery($query);
                $userRecommends = $db->loadObjectList();
                foreach ($userRecommends as $key => $userRecommend) {
                    $userRecommendPhone[$userRecommend->id] = $userRecommend->phone;
                }
         ?>
        <tbody>
            <?php $nrcrt = 1; $i=0;
            ...
            ?>
                <TR class="row<?php echo $i % 2; ?>">
                    ...
                    <TD align="left"><a
                        href='<?php echo JRoute::_( 'index.php?option=com_jbusinessdirectory&task=spepoint.edit&id='. $item->id )?>'
                    ...
                    <td>
                        <?php echo $speStoreName[$item->userId] ?>
                    </td>
                    <td>
                        <?php echo $item->phone ?>
                    </td>
                    ...
                    <td>
                        <?php
                           if (!isset($speSendPoint[$item->userId])) {
                                echo 0;
                            }
                            echo @$speSendPoint[$item->userId];
                        ?>
                    </td>
                    <td>
                         <?php
                    ...
                    <!-- <td valign="top" align="center">
                        <img
                            ...
                            onclick="document.location.href = '<?php echo JRoute::_( 'index.php?option=com_jbusinessdirectory&task=spepoints.chageState&id='. $item->id )?> '"
                        />
                    </td> -->
