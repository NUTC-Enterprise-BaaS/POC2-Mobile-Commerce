	...
	<table class="table table-striped adminlist" id="itemList">
		<thead>
			<tr>
				...
				<th class="hidden-phone"><?php echo JHtml::_('grid.sort', 'LNG_FIRSTNAME', 'rr.firstName', $listDirn, $listOrder); ?></th>
				<th class="hidden-phone"><?php echo JHtml::_('grid.sort', 'LNG_LASTNAME', 'rr.lastName', $listDirn, $listOrder); ?></th>
				<th><?php echo JHtml::_('grid.sort', 'LNG_EMAIL', 'rr.email', $listDirn, $listOrder); ?></th>
				<th><?php echo JHtml::_('grid.sort', 'LNG_STATE', 'rr.state', $listDirn, $listOrder); ?></th>
				<th class="hidden-phone"><?php echo JHtml::_('grid.sort', 'LNG_ID', 'rr.id', $listDirn, $listOrder); ?></th>
			</tr>
		</thead>
		<tfoot>
			<tr>
		...
		<tbody>
			...
			foreach($this->items as $reviewresponse) { ?>
				<tr class="row<?php echo $nrcrt%2?>"  >
					...
					<td align='center' class="hidden-phone"><?php echo JHtml::_('grid.id', $nrcrt, $reviewresponse->id); ?></td>
					<td align='left'>
					...
					<td class="hidden-phone"><?php echo $reviewresponse->firstName ?></td>
					<td class="hidden-phone"><?php echo $reviewresponse->lastName ?></td>
					<td><?php echo $reviewresponse->email ?></td>
					<td valign='top' align='center'>
						<img src="<?php echo JURI::base() ."components/".JBusinessUtil::getComponentName()."/assets/img/".($reviewresponse->state==0? "unchecked.gif" : "checked.gif")?>" 
						onclick="document.location.href = '<?php echo JRoute::_( 'index.php?option=com_jbusinessdirectory&task=reviewresponses.chageState&id='. $reviewresponse->id )?> '"/>
					</td>
					<td class="hidden-phone"><?php echo $reviewresponse->id ?></td>
				</tr>
			<?php } ?>
		</tbody>
	</table>
