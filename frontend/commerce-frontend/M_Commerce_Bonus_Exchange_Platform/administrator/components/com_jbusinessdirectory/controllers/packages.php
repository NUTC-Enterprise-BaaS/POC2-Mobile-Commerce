...
class JBusinessDirectoryControllerPackages extends JControllerAdmin
{
	/**
	 * Display the view
	 *
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Package', $prefix = 'JBusinessDirectoryModel', $config = array('ignore_request' => true))
	{
	...
	public function delete()
	{
		...
		$this->setRedirect('index.php?option=com_jbusinessdirectory&view=packages');
	}
	...
	/**
	 * Change poular state of selected item
	 */
	function chagePopularState(){
		$model = $this->getModel('Package');
	
		if (!$model->changePopularState()){
			$this->setMessage(JText::_('LNG_ERROR_CHANGE_STATE'), 'warning');
		}
