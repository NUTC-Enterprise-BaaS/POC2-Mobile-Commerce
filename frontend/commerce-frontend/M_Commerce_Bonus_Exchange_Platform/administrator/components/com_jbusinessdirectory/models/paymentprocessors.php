 ...
 * @subpackage  com_jbusinessdirectory
 */
class JBusinessDirectoryModelPaymentProcessors extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getItems()
	{
		...
		$table = JTable::getInstance("PaymentProcessorFields", "JTable");
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__jbusinessdirectory_payment_processors').' AS p');
		
		// Filter by search in title.
		$search = $this->getState('filter.search');
		if (!empty($search)) {
			$query->where("p.name LIKE '%".trim($db->escape($search))."%' or 
							p.type LIKE '%".trim($db->escape($search))."%'");
		}
