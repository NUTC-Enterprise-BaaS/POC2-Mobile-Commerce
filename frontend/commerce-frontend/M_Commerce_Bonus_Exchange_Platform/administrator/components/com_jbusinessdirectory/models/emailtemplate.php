...
class JBusinessDirectoryModelEmailTemplate extends JModelAdmin
{
	...
	*/
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jbusinessdirectory.emailtemplate', 'item', array('control' => 'jform', 'load_data' => $loadData), true);
		if (empty($form))
		{
			return false;
		}
	...
	{
		$id	= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('emailtemplate.id');
		$isNew = true;
