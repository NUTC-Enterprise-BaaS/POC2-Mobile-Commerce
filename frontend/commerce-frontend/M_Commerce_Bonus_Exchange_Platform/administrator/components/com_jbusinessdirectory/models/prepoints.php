 ...
 */
class JBusinessDirectoryModelPrepoints extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__jbusinessdirectory_companies').' AS p');
		$query->where($db->quoteName('shop_class') . ' = ' . 2);

		// Filter by search in title.
		$search = $this->getState('filter.search');
		if (!empty($search)) {
			$query->where("p.address LIKE '%".trim($db->escape($search))."%' or
                            p.id LIKE '%".trim($db->escape($search))."%'or
                            p.phone LIKE '%".trim($db->escape($search))."%'or
                            p.name LIKE '%".trim($db->escape($search))."%'");
			$query->where($db->quoteName('shop_class') . ' = ' . 2);
		}
