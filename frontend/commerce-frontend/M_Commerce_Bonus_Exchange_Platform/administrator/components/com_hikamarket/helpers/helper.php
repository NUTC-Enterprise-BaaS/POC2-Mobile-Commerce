...
class hikamarket {
	...
	public static function footer() {
		...
		if(JRequest::getInt('marketbuild',0)) $text .= '<!-- 1605021210 -->';
	...
	public static function isEditableOrder($order_id) {
		...
		$query = 'SELECT o.order_type, COUNT(DISTINCT op.order_vendor_id) as vendor_count '.
			...
			' LEFT JOIN ' . hikamarket::table('shop.order') . ' AS op ON (o.order_parent_id > 0 AND o.order_parent_id = op.order_parent_id) ' .
			' WHERE (o.order_id = ' . (int)$order_id . ') AND (op.order_type = ' . $db->Quote('subsale') . ')';
		$db->setQuery($query);
		$ret = $db->loadObject();
