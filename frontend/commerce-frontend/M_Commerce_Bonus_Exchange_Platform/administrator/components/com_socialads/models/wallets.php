 ...
 * @since  1.6
 */
class SocialadsModelWallets extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__ad_wallet_transc` AS a');
		$query->select('username AS created_by, u.email');
		$query->join('INNER', '#__users AS u ON a.user_id = u.id');
		$query->where('a.time = (select MAX(time) FROM #__ad_wallet_transc WHERE user_id = a.user_id)');
