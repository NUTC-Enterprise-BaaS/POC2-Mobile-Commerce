...
?>
		<tr class="hikashop_order_additional_additional">
			...
			$query->where($db->quoteName('message') . ' LIKE '. $db->quote('訂單編號 : '.$this->order->order_number));
			$db->setQuery($query);
			$results = $db->loadObjectList();
			$finalPrice = $this->order->order_subtotal + -($results[0]->points);
			if(!empty($additional->order_product_price)) {
				// $additional->order_product_price = (float)$additional->order_product_price;
				$additional->order_product_price = -($results[0]->points); //改為扣的點數
			}
...
?>
		<tr class="hikashop_order_additional_total">
			...
			<td><span>
			<?php
			if ($finalPrice != null) { //	判斷顯示的價格
				echo $this->currencyHelper->format($finalPrice,$this->order->order_currency_id);
			} else {
