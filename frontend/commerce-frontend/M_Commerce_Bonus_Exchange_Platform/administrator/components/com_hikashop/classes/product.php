	...
	public function backSaveForm() {
		...
		foreach($priceData as $k => $value) {
			...
			if(isset($value['price_value']))
				...
				$discount = (int)JRequest::getVar('price_with_discount_0');
				if ($discount > 100) {
					$product->prices[$k]->price_value_with_discount = 0;
				}
				elseif ($discount <= 0) {
					$product->prices[$k]->price_value_with_discount = 0;
				}
				else {
					$product->prices[$k]->price_value_with_discount = $discount;
				}
				$product->prices[$k]->price_value = hikashop_toFloat($value['price_value']);
					//存入原價
	...
	function save(&$element, $stats = false) {
		...
		if(empty($element->product_id) && empty($element->product_code)) {
			if(!empty($element->product_name)) {
				$search = explode(',', 'c,a,?,a,e,i,o,u,a,e,i,o,u,a,e,i,o,u,y,a,e,i,o,u,a,e,i,o,u');
	...
	function updatePrices($element,$status){
		...
		if(count($element->prices)){
			...
			foreach($element->prices as $price){
				...
				$line = '('.(int)$price->price_currency_id.','.$status.','.(int)$price->price_min_quantity.','.(float)$price->price_value.','.$price->price_id.','.$price->price_value_with_discount.','.$this->database->Quote(@$price->price_site_id);
				if(hikashop_level(2)){
			...
			if(!empty($insert)){
				...
				price_value_with_discount,price_site_id';
				if(hikashop_level(2)){
					$select.=',price_access';
				}
