...
class hikashopFilterTypeClass extends hikashopClass {
			...
			if(!is_null($new_filter) && $new_filter!=$oldValue && (!$config->get('redirect_post',0) || $new_filter_with_post)){
				...
				if($this->params->get('content_type')!='product'){
					...
					foreach($this->modules as $module){
						JRequest::setVar($base.$module->id.'_'.$cid,0);
						JRequest::setVar($base.$module->id,0);
					}
					JRequest::setVar($base.'_'.$cid,0);
				}else{
	...
	function getDeleteButton($filter, $divName, $val, $html, $br, $onlyButton=false, $id=''){
		...
		if($onlyButton){
			...
			return '<a href="#" style="cursor:pointer; text-decoration:none" onclick="document.getElementById(\''.$id.'\').value=\' \'; '.$submitOnClick.'return false;"><img src="'.HIKASHOP_IMAGES.'delete2.png" /></a>';
		}
		...
		$html.='  <a href="#" style="cursor:pointer; text-decoration:none" onclick="document.getElementById(\'filter_'.$filter->filter_namekey.'_'.$divName.'\').value=\' \'; '.$submitOnClick.'return false;"><img src="'.HIKASHOP_IMAGES.'delete2.png" /></a><br/>';
		return $html;
	}
	...
	function getManufacturers($filter, $datas=''){
		...
		$filters = array('a.category_type="manufacturer"','a.category_parent_id!=\'1\' '.$optionElement,'a.category_published=1');
		hikashop_addACLFilters($filters,'category_access','a');
		$database = JFactory::getDBO();
...
hkjQuery(document).ready(function(){
	...
	function cursorTitlePosition($filter, $html, $width, $divName,$deleteButton,$hasValue){
		...
		$input.='<span id="filter_span_'.$filter->filter_namekey.'_'.$divName.'" class="hikashop_filter_cursor_span"></span>'.$deleteButton;
		if($filter->filter_data=='weight' || $filter->filter_data=='length' || $filter->filter_data=='width' || $filter->filter_data=='height' || $filter->filter_data=='surface' || $filter->filter_data=='volume'){
