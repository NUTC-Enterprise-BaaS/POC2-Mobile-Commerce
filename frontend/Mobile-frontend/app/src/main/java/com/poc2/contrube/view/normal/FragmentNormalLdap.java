...
import com.poc2.contrube.controllor.NormalLdapController;
...
public class FragmentNormalLdap extends Fragment {
    ...
    private void findView() {
        ...
        mTokenButton.setOnClickListener(createClickEvent);
    }
    ...
    private View.OnClickListener submitClickEvent = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (mTokenEditText.getText().toString().equals("") || mTokenEditText.getText().toString().length() != 4) {
                Toast.makeText(getContext(), "請填寫正確的驗證碼", Toast.LENGTH_LONG).show();
                return;
            }
            controller.connectLdap(mTokenEditText.getText().toString(),mPreferencesHelperImp.getLDAPToken());
        }
    };
    ...
    private NormalLdapController.CallBackEvent callBackEvent = new NormalLdapController.CallBackEvent() {
        ...
        public void onSuccess(ApiV1NormalConnectLdapPostData information) {
            if (information.message.contains("success")) {
                ...
                Toast.makeText(getContext(), "驗證碼已經過期，請再次產生一組新的序號", Toast.LENGTH_LONG).show();
            }
        }

        @Override
        public void onSuccess(ApiV1NormalTokenGetData information) {
            mTextToken.setText("您的驗證碼：" + information.code);
        }
